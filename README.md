# php_tutorial
php - Статьи
- # [PHP — P1 Basic Syntax](https://github.com/MrGoodo/php_tutorial/blob/master/PHP%20%E2%80%94%20P1%20Basic%20Syntax.php)
...
- # [PHP — P2 Comments](https://github.com/MrGoodo/php_tutorial/blob/master/PHP%20%E2%80%94%20P2%20Comments.php)
...
- # [PHP — P3: Variables Intro](https://github.com/MrGoodo/php_tutorial/blob/master/PHP%20%E2%80%94%20P3%20Variables%20Intro.php)
...
- # [PHP — P4: Basic Booleans](https://github.com/MrGoodo/php_tutorial/blob/master/PHP%20%E2%80%94%20P4%20Basic%20Booleans.php)
...
__Напишем еще несколько переменных, подумай как они работают, самое главное проверь лично, попробуй изменить код, написать по-другому, в коде главное ПРАКТИКА, все что я тут пишу, проверь сам, измени условия, сделай все по-своему, чтобы лучше усвоить.__
```php
...
```
Конец, последний блок кода, самый частый способ использовать booleans, использовать с IfElseIf утверждениями, про это узнаем дальше.

- # [PHP — P5: Truthy Bools](https://github.com/MrGoodo/php_tutorial/blob/master/PHP%20%E2%80%94%20P5%20Truthy%20Bools.php)

__Мы рассмотрели логические значения впредыдущей статье. Логические значения обозначают значение true или false. PHP имеет правдивые логические значения, которые не являются истинными или ложными, но при вычислении внутри выражения они могут действовать так, как есть.__

__Лучшими примерами являются целые числа 0 и 1. Если бы мы передали 1 оператору if, PHP интерпретировал бы число 1 как истинное и выполнил бы оператор внутри тела. Если передать 0 оператору if, PHP будет интерпретировать 0 как ложный, поэтому он не будет выполнять инструкцию внутри тела оператора if.__

__На самом деле, мы можем добавить любое целое значение, положительное или отрицательное, и PHP будет интерпретировать их как правдивые. Он интерпретирует только 0 как ложный.__
``` php
        if ( 1 ) {
            echo "He was number 1";
        }
```
Данный код выведет нам текст, так как единица(1) это true, поэтому текст будет выведет в браузер.
```php
        if ( -5 ){
            echo "He was also number 1";
        }
```
Данный код также будет считаться как true, так как любое число, отрицательное или положительное, выведет true, только в одном случае будет false, это если число будет равно нулю (0)
```php
        if ( 0 ){
            echo "Was he number 1?";
        }
```
Данный текст в браузер не будет выведен, так как это в условии у нас цифра ноль(0), и поэтому утверждение равно false,
```php
        $cars = []; // empty array
        if ( $cars ) {
...
```
▲ Пустые массивы, в коде воспринимаются как false, поэтому данный текст в данном утверждении не будет выведен в браузер!
...
```php
        var_dump( (bool) "false" );
```
▲ Выведет true, так как это все равно обычный стринг, и не важно что в нем написано, он существует, значит выведет!
```php
        var_dump( (bool) [] );
```
▲ Пустой массив, будет false, как и в примере выше.
```php
        var_dump( (bool) $names );
```
▲ Тут var_dump(Имя нашего массива) обращается к нашему массиву, в котором есть какое то количество элементов, поэтому данное утверждение будет true.
```php
        if ( count($names) > 0 ){
            echo "This is a true statement";
        }
```
▲ Данное утверждение обращается к нашему массиву и считает его элементы, и ставит условие, если колличество элементов массива, будет больше нуля(0), то текст будет выведен, и в данном случае, текст действительно будет выведен!
